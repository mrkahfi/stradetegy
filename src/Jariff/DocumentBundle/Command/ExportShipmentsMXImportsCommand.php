...
class ExportShipmentsMXImportsCommand extends ContainerAwareCommand
{

    protected function configure()
    {
        $this
            ->setName('jariff:cron:export-shipments-mx-imports')
            ->setDescription('Mengeksekusi file export shipments MX IM')
            ->setHelp('<<<EOF
            <info>jariff:cron:export</info>
            Command digunakan untuk menyimpan data pencarian

            Syntax :
            php app/console jariff:cron:export-shipments-mx-imports
            EOF'
            );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $fp = fopen(getcwd() . '/lock/jariff-export-mx-imports.lock', 'w');
        ...
        if (flock($fp, LOCK_EX | LOCK_NB)) {
            ...
            echo "mulai\n";
            $exportDownloadQueque = $em->getRepository('JariffMemberBundle:ExportTools')->findOneBy(array('process' => 1,'collection' => 'mx-imports'));
            ...
            ->setTitle("Shipments Data MX Imports");

            $fields = array("hs_product_dectiption",
...
//            $highlight,
            ...
            $searchable = new Index($client, 'stradetegy-im-mx');
            ...
            $columns = array();
            $cell = $phpExcelObject->createSheet(0)->setTitle('Shipments MX Imports');
            $i = 0;
            foreach ($column as $col) {
                $columns[$util->mx_imports_column_view($col)] = $util->rowXls($i) . '1';
                $cell->setCellValue($util->rowXls($i) . '1', $util->mx_imports_column_view($col));
                $i++;
            }
