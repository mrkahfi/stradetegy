...
class ExportShipmentsJNPTImportsCommand extends ContainerAwareCommand
{

    protected function configure()
    {
        $this
            ->setName('jariff:cron:export-shipments-jnpt-imports')
            ->setDescription('Mengeksekusi file export shipments JNPT IM')
            ->setHelp('<<<EOF
            <info>jariff:cron:export</info>
            Command digunakan untuk menyimpan data pencarian

            Syntax :
            php app/console jariff:cron:export-shipments-jnpt-imports
            EOF'
            );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $fp = fopen(getcwd() . '/lock/jariff-export-jnpt-imports.lock', 'w');
        ...
        if (flock($fp, LOCK_EX | LOCK_NB)) {
            ...
            echo "mulai\n";
            $exportDownloadQueque = $em->getRepository('JariffMemberBundle:ExportTools')->findOneBy(array('process' => 1,'collection' => 'jnpt-imports'));
            ...
            ->setTitle("Shipments Data JNPT Imports");

            $fields = array("importer_name",
            "importer_address",
            "city",
            "zip_cod",
            "item_description",
...
//            $highlight,
            ...
            $searchable = new Index($client, 'stradetegy-im-jnpt');
            ...
            $columns = array();
            $cell = $phpExcelObject->createSheet(0)->setTitle('Shipments JNPT Imports');
            $i = 0;
            foreach ($column as $col) {
                $columns[$util->jnpt_imports_column_view($col)] = $util->rowXls($i) . '1';
                $cell->setCellValue($util->rowXls($i) . '1', $util->jnpt_imports_column_view($col));
                $i++;
            }
