...
use Jariff\MemberBundle\Form\SearchGlobal\Shipments\CLImports\FieldCLImportFormType;
 ...
 * @Route("/member")
 */
class SearchShipmentsCLImportsController extends BaseController
{
    ...
    /**
     * @Route(
     *   "/search-shipments-result/cl-imports/{page}",
     *   name     = "member_search_shipments_result_cl_imports",
     *   defaults = {
     *     "page"  : "1"
     *   },
     *   requirements = {
     ...
     * )
     *
     * @Template("JariffMemberBundle:Search\ShipmentsClImports:results.html.twig")
     */
...
$formFieldUsCustom = $this->createForm(new FieldCLImportFormType(), $fieldUsCustom, array(
    'action' => '',
    'method' => 'POST',
    ));
...
return array(
    ...
    /**
     * @Route(
     *   "/search-shipments-result-ajax/cl-imports/{page}",
     *   name     = "member_search_shipments_result_cl_imports_ajax",
     *   defaults = {
     *     "page"  : "1"
     *   },
     *   requirements = {
     ...
     * )
     *
     * @Template("JariffMemberBundle:Search\ShipmentsClImports\Add:results_ajax.html.twig")
     */
    public function resultAjaxAction($page = 1)
    {
        ...
        $searchable = new Index($client, 'stradetegy_cl_imports');
        ...
        $html = $view->render($res, function ($page) {
            return $this->generateUrl('member_search_shipments_result_cl_imports_ajax', array('page' => $page, 's_cache' => $this->s_cache_json));
        }, $options);
        ...
        $fieldUsCustom = new FieldUsCustom();
        if (empty($s_cache->column))
            $fieldUsCustom->field_us_custom = array("date","importer", "hs_code",
                "custom", "product", "variety", "brand", "quantity",  "gross_weight");
        else {
            $fieldUsCustom->field_us_custom = (is_array($s_cache->column)) ? $s_cache->column : array($s_cache->column);
        }
        ...
        return new Response(
            json_encode(
                array(
                    'success' => true,
                    'html_string' => $this->renderView('JariffMemberBundle:Search\ShipmentsClImports\Add:results_ajax.html.twig', $params),
                    's_cache' => $this->s_cache_json,
                    'url_changes' => $this->generateUrl('member_search_shipments_result_cl_imports', array('page' => $page, 's_cache' => $this->s_cache_json))
                    )
                )
            );
    }
    ...
    /**
     * @Route(
     *   "/search-shipments-columns-submit-cl-imports",
     *   name     = "member_search_columns_category_submit_cl_imports"
     * )
     * @Method("POST")
     *
     */
    public function submitCategoryAction(Request $request)
    {
        ...
        $form = $this->createForm(new FieldCLImportFormType(), $entity, array(
            'action' => '',
            'method' => 'POST',
            ));
        ...
        return $this->redirectUrl('member_search_shipments_result_cl_imports', array('s_cache' => $s_cache));

    }

    /**
     * @Route(
     *   "/search-shipments-columns-show-cl-imports",
     *   name     = "member_search_columns_category_columns_show_cl_imports"
     * )
     *
     *
     */
    ...
    /**
     * @Route(
     *   "/company-display-one-shipments-cl-imports/{index}/{type}/{objId}",
     *   name     = "company_display_one_shipments_cl_imports",
     *  options={"expose"=true}
     * )
     *
     *
    ...
    function displayOneShipmentsAction($index, $type, $objId)
    {
        ...
        return new Response(
            json_encode(
                array(
                    'success' => true,
                    'html_string' => $this->renderView('JariffMemberBundle:Search\ShipmentsClImports\Add:modal_show_one_shipments.html.twig', array('entity' => $ret,'params' => $params)),
    ...
    /**
     * @Route(
     *   "/search-shipments-result-ajax/cl-imports/jsonp",
     *   name     = "member_search_shipments_result_cl_imports_ajax_jsonp",
     *   defaults = {
     *     "page"  : "1"
     *   },
     *   requirements = {
...
if ($res->getNbResults() == 0) {
    ...
    $inbound->setVisitedPage($this->generateUrl('member_search_shipments_result_cl_imports', array('s_cache' => $this->s_cache_json)));
...
if ($maxTime > 1) {
    ...
    $urlChanges = $this->generateUrl('member_search_shipments_result_cl_imports', array('page' => $page, 's_cache' => $this->s_cache_json));
...
foreach ($res->getCurrentPageResults()->getResults() as $r) {
    ...
    data-url="' . $this->generateUrl('company_display_one_shipments_cl_imports', array(
