...
class ExportShipmentsPAImportsCommand extends ContainerAwareCommand
{

    protected function configure()
    {
        $this
        ->setName('jariff:cron:export-shipments-pa-imports')
        ...
        ->setHelp('<<<EOF
            ...
            Syntax :
            php app/console jariff:cron:export-shipments-pa-imports
            EOF'
            );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $fp = fopen(getcwd() . '/lock/jariff-export-pa-imports.lock', 'w');
        ...
        if (flock($fp, LOCK_EX | LOCK_NB)) {
            ...
            echo "mulai\n";
            $exportDownloadQueque = $em->getRepository('JariffMemberBundle:ExportTools')->findOneBy(array('process' => 1,'collection' => 'pa-imports'));
            ...
            ->setTitle("Shipments Data PA Imports");
...
//            $highlight,
            ...
            $searchable = new Index($client, 'stradetegy-im-pa');
            ...
            $columns = array();
            $cell = $phpExcelObject->createSheet(0)->setTitle('Shipments PA Imports');
            $i = 0;
            foreach ($column as $col) {
                $columns[$util->pa_imports_column_view($col)] = $util->rowXls($i) . '1';
                $cell->setCellValue($util->rowXls($i) . '1', $util->pa_imports_column_view($col));
                $i++;
            }
