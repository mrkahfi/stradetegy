var mapImporter = function () {
    ...
    emit({slug_shipper_name: value.slug_shipper_name, slug_consignee_name: value.slug_consignee_name}, values);
}
...
var resSup1 = db.import_document_rewrite.mapReduce(mapImporter, reduceImporter,
    { query: {"value.shipper_name": {$ne: "-NOT AVAILABLE-"},
        $or: [
            { "value.shipper_name": {$ne: ""}}
        ]}, out: "import_document_importer"});
...
var resSup1 = db.import_document_rewrite.mapReduce(mapImporter, reduceImporter,
    { query: {"value.shipper_name": {$ne: "-NOT AVAILABLE-"},
        $or: [
            { "value.shipper_name": {$ne: ""}}
        ]}, out: "import_document_importer_part_1"});

db.import_document_importer_part_1.find().forEach(
    function (obj) {
        db.import_document_importer.update({'_id': obj._id}, {$set: {
            'value.carrier': obj.value.carrier,
            'value.vessel': obj.value.vessel,
            'value.container_number': obj.value.container_number,
            'value.country_of_origin': obj.value.country_of_origin,
