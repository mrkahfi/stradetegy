...
class ExportShipmentsCLExportsCommand extends ContainerAwareCommand
{

    protected function configure()
    {
        $this
            ->setName('jariff:cron:export-shipments-cl-exports')
            ->setDescription('Mengeksekusi file export shipments CL EX')
            ->setHelp('<<<EOF
            <info>jariff:cron:export</info>
            Command digunakan untuk menyimpan data pencarian

            Syntax :
            php app/console jariff:cron:export-shipments-cl-exports
            EOF'
            );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $fp = fopen(getcwd() . '/lock/jariff-export-cl-exports.lock', 'w');
        ...
        if (flock($fp, LOCK_EX | LOCK_NB)) {
            ...
            echo "mulai\n";
            $exportDownloadQueque = $em->getRepository('JariffMemberBundle:ExportTools')->findOneBy(array('process' => 1,'collection' => 'cl-exports'));
            ...
            ->setTitle("Shipments Data CL Imports");
...
//            $highlight,
            ...
            $searchable = new Index($client, 'stradetegy-ex-cl');
            ...
            $columns = array();
            $cell = $phpExcelObject->createSheet(0)->setTitle('Shipments CL Exports');
            $i = 0;
            foreach ($column as $col) {
                $columns[$util->cl_exports_column_view($col)] = $util->rowXls($i) . '1';
                $cell->setCellValue($util->rowXls($i) . '1', $util->cl_exports_column_view($col));
                $i++;
            }
