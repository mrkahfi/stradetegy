...
class ExportShipmentsVEImportsCommand extends ContainerAwareCommand
{

    protected function configure()
    {
        $this
            ->setName('jariff:cron:export-shipments-ve-imports')
            ->setDescription('Mengeksekusi file export shipments VE IM')
            ->setHelp('<<<EOF
            <info>jariff:cron:export</info>
            Command digunakan untuk menyimpan data pencarian

            Syntax :
            php app/console jariff:cron:export-shipments-ve-imports
            EOF'
            );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $fp = fopen(getcwd() . '/lock/jariff-export-ve-imports.lock', 'w');
        ...
        if (flock($fp, LOCK_EX | LOCK_NB)) {
            ...
            echo "mulai\n";
            $exportDownloadQueque = $em->getRepository('JariffMemberBundle:ExportTools')->findOneBy(array('process' => 1,'collection' => 've-imports'));
            ...
            ->setTitle("Shipments Data VE Imports");

            $fields = array("chapter",
            "chapter_description",
            "importer",
            "custom",
            "transport_type",
...
//            $highlight,
            ...
            $searchable = new Index($client, 'stradetegy-im-ve');
            ...
            $columns = array();
            $cell = $phpExcelObject->createSheet(0)->setTitle('Shipments Ve Imports');
            $i = 0;
            foreach ($column as $col) {
                $columns[$util->ve_imports_column_view($col)] = $util->rowXls($i) . '1';
                $cell->setCellValue($util->rowXls($i) . '1', $util->ve_imports_column_view($col));
                $i++;
            }
