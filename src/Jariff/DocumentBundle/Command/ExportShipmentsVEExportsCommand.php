...
class ExportShipmentsVEExportsCommand extends ContainerAwareCommand
{

    protected function configure()
    {
        $this
            ->setName('jariff:cron:export-shipments-ve-exports')
            ->setDescription('Mengeksekusi file export shipments VE EX')
            ->setHelp('<<<EOF
            <info>jariff:cron:export</info>
            Command digunakan untuk menyimpan data pencarian

            Syntax :
            php app/console jariff:cron:export-shipments-ve-exports
            EOF'
            );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $fp = fopen(getcwd() . '/lock/jariff-export-ve-exports.lock', 'w');
        ...
        if (flock($fp, LOCK_EX | LOCK_NB)) {
            ...
            echo "mulai\n";
            $exportDownloadQueque = $em->getRepository('JariffMemberBundle:ExportTools')->findOneBy(array('process' => 1,'collection' => 've-exports'));
            ...
            ->setTitle("Shipments Data VE Exports");
...
//            $highlight,
            ...
            $searchable = new Index($client, 'stradetegy-ex-ve');
            ...
            $columns = array();
            $cell = $phpExcelObject->createSheet(0)->setTitle('Shipments VE Exports');
            $i = 0;
            foreach ($column as $col) {
                $columns[$util->ve_exports_column_view($col)] = $util->rowXls($i) . '1';
                $cell->setCellValue($util->rowXls($i) . '1', $util->ve_exports_column_view($col));
                $i++;
            }
