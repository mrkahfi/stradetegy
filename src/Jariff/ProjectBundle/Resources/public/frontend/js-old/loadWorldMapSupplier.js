...
function init(slugParam, pageParam) {
    ...
    var url = Routing.generate('world_map_supplier_json', { slug : slugParam, page : pageParam });
        ...
        $.getJSON(url, function (json) {
            ...
            var ht = new $jit.Hypertree({
                ...
                onPlaceLabel: function (domElement, node) {
                    ...
                    var address = node.data.consignee_address;
                    var containerCount = node.data.consigneeContainerCount;
                    if (node._depth == 0) {
                    ...
                    for (var i = 0; i < newSplittedWords.length; i++) {
                        ...
                        if (contentsOfFileAsString.indexOf('name="' + split.toUpperCase() + '"') !== -1) {
                            ...
                            isFound = true;  /* console.log('COUNTRY  ==  ' + country); */
                    ...
                    if (country.length > 1) {
                        ...
                        if (typeof(stack.add != undefined)) {
                            ...
                            type = isOdd(node._depth) ? 'Supplier' : 'Supplier';
                            html = "<div id='node-info'><div id='company-name'><span style='font-size: 120%;'><strong>"+node.name+"</strong></span></div>Company Type<div id='company-type'><span style='font-size:120%;'>" + type +"</span></div>Featured Address<div id='featured-address'><span style='font-size:120%;'>"+ ((node._depth > 0) ? node.data.consignee_address : node.data.address) +"</span></div>Connections<div id='connection-count'><span style='font-size:120%;'>"+connCount+"</span></div>Showing Connections<div id='showing-connection'><span style='font-size:120%;'>"+showingCount+"</span></div></div>";

                            if (node.id != slugParam) {
                                html2 = "<p><input style='margin-right:10px; vertical-align:middle' type='checkbox' "+ (isChecked ? "checked='true'" : '') + " id='hide-labels' /><span>Show hidden suppliers</span></p><p>See more connections by using <b>Set as Root</b></p><p><button onclick=\"init('"+node.id+"', '0')\" class='button button-search global-button-search' >Set as Root</button>"+"<p><button onclick=\"Clickheretoprint();\" class='button button-search global-button-search' >Printer Friendly Version</button>";
                            } else {
                                html2 = "<p><input style='margin-right:10px; vertical-align:middle' type='checkbox' "+ (isChecked ? "checked='true'" : '') + " id='hide-labels' /><span>Show hidden suppliers</span></p><p>See more connections by using <b>Load Next Nodes</b></p><p><button onclick=\"init('"+slugParam+"', '"+(parseInt(pageParam)+1)+"')\" class='button button-search global-button-search' >Load Next Nodes</button>"+"<p><button onclick=\"Clickheretoprint();\" class='button button-search global-button-search' >Printer Friendly Version</button>";
                            }
                            console.log('COUNTRY == '+country);
...
function plotMarker() {
    ...
    console.log('\n\n\nnormalContainerCount '+normalContainerCount+'\n');
    ...
    for (var i = 0; i < markersArray.length; i++) {
        if (markersArray[i] != undefined) {
            ...
            console.log('total Count of ' + countryCache[i] + ' : ' + conCount + '. Berarti lum-nya : ' + lum);
...
function getAddress(search, node, html, stack) {
    geocoder.geocode({ 'address': search }, stack.add(function(results, status) {
        if (status == google.maps.GeocoderStatus.OK) {
            ...
            console.log(node.name + ' ... ' + node._depth + ' ... ' + search + ' ... '+results[0].geometry.location);
        } else {
