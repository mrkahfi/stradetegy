 ...
 * @subpackage AuthorizeNetDPM
 */
class DPM extends AuthorizeNet\API\SIMForm
{
    ...
    public static function directPostDemo($url, $api_login_id, $transaction_key, $amount = "0.00", $md5_setting = "")
    {
        ...
        if (!count($_POST) && !count($_GET))
        {
            ...
            echo DPM::getCreditCardForm($amount, $fp_sequence, $url, $api_login_id, $transaction_key);
        }
        // Step 2: Handle AuthorizeNet Transaction Result & return snippet.
        elseif (count($_POST)) 
        {
            $response = new SIM($api_login_id, $md5_setting);
            if ($response->isAuthorizeNet()) 
            {
                ...
                echo DPM::getRelayResponseSnippet($redirect_url);
            }
            else
            {
