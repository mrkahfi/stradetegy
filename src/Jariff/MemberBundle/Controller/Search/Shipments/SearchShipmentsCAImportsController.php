...
use Jariff\MemberBundle\Form\SearchGlobal\Shipments\CAImports\FieldCAImportFormType;
 ...
 * @Route("/member")
 */
class SearchShipmentsCAImportsController extends BaseController
{
    ...
    /**
     * @Route(
     *   "/search-shipments-result/ca-imports/{page}",
     *   name     = "member_search_shipments_result_ca_imports",
     *   defaults = {
     *     "page"  : "1"
     *   },
     *   requirements = {
     ...
     * )
     *
     * @Template("JariffMemberBundle:Search\ShipmentsCaImports:results.html.twig")
     */
    public function resultAction($page = 1)
    {
        ...
        if (empty($s_cache->column))
            $fieldUsCustom->field_us_custom = array("date", "hs_code",
                "spanish_description", "province", "tipes", "origin_country",
                "state_of_origin", "quantity", "comercial_unit", "import_fob_value",
                "port_of_entry", "hs_code_description");
        else {
            $fieldUsCustom->field_us_custom = (is_array($s_cache->column)) ? $s_cache->column : array($s_cache->column);
        }

        $formFieldUsCustom = $this->createForm(new FieldCAImportFormType(), $fieldUsCustom, array(
            'action' => '',
            'method' => 'POST',
            ));
    ...
    /**
     * @Route(
     *   "/search-shipments-result-ajax/ca-imports/{page}",
     *   name     = "member_search_shipments_result_ca_imports_ajax",
     *   defaults = {
     *     "page"  : "1"
     *   },
     *   requirements = {
     ...
     * )
     *
     * @Template("JariffMemberBundle:Search\ShipmentsCaImports\Add:results_ajax.html.twig")
     */
    public function resultAjaxAction($page = 1)
    {
        ...
        $searchable = new Index($client, 'stradetegy_ca_imports');
        ...
        $html = $view->render($res, function ($page) {
            return $this->generateUrl('member_search_shipments_result_ca_imports_ajax', array('page' => $page, 's_cache' => $this->s_cache_json));
        }, $options);
        ...
        return new Response(
            json_encode(
                array(
                    'success' => true,
                    'html_string' => $this->renderView('JariffMemberBundle:Search\ShipmentsCaImports\Add:results_ajax.html.twig', $params),
                    's_cache' => $this->s_cache_json,
                    'url_changes' => $this->generateUrl('member_search_shipments_result_ca_imports', array('page' => $page, 's_cache' => $this->s_cache_json))
                    )
                )
            );
    }
    ...
    /**
     * @Route(
     *   "/search-shipments-columns-submit-ca-imports",
     *   name     = "member_search_columns_category_submit_ca_imports"
     * )
     * @Method("POST")
     *
     */
    public function submitCategoryAction(Request $request)
    {
        ...
        $form = $this->createForm(new FieldCaImportFormType(), $entity, array(
            'action' => '',
            'method' => 'POST',
            ));
        ...
        return $this->redirectUrl('member_search_shipments_result_ca_imports', array('s_cache' => $s_cache));

    }

    /**
     * @Route(
     *   "/search-shipments-columns-show-ca-imports",
     *   name     = "member_search_columns_category_columns_show_ca_imports"
     * )
     *
     *
     */
    public function showChoiceColoumnAction(Request $request)
    {
        ...
        return new Response(
            json_encode(
                array(
                    'success' => true,
                    'html_string' => $this->renderView('JariffMemberBundle:Search\ShipmentsCaImports\Add\Form:field_column.html.twig', $params),
    ...
    /**
     * @Route(
     *   "/company-display-one-shipments-ca-imports/{index}/{type}/{objId}",
     *   name     = "company_display_one_shipments_ca_imports",
     *  options={"expose"=true}
     * )
     *
     *
    ...
    function displayOneShipmentsAction($index, $type, $objId)
    {
        ...
        return new Response(
            json_encode(
                array(
                    'success' => true,
                    'html_string' => $this->renderView('JariffMemberBundle:Search\ShipmentsCaImports\Add:modal_show_one_shipments.html.twig', array('entity' => $ret,'params' => $params)),
    ...
    /**
     * @Route(
     *   "/search-shipments-result-ajax/ca-imports/jsonp",
     *   name     = "member_search_shipments_result_ca_imports_ajax_jsonp",
     *   defaults = {
     *     "page"  : "1"
     *   },
     *   requirements = {
    ...
    public function resultAjaxJsonpAction()
    {
        ...
        $fields = array("hs_code",
            "spanish_description", "province", "tipes", "origin_country",
            "state_of_origin", "quantity", "comercial_unit", "import_fob_value",
            "port_of_entry", "hs_code_description");


        $queryDSL = $this->queryDSL($s_cache, "date", "hs_code_description", $fields, $size, $page);
...
//        }
        ...
        if ($res->getNbResults() == 0) {
            ...
            $inbound->setVisitedPage($this->generateUrl('member_search_shipments_result_ca_imports', array('s_cache' => $this->s_cache_json)));
        ...
        if ($maxTime > 1) {
            ...
            $urlChanges = $this->generateUrl('member_search_shipments_result_ca_imports', array('page' => $page, 's_cache' => $this->s_cache_json));
        ...
        foreach ($res->getCurrentPageResults()->getResults() as $r) {
            ...
            data-url="' . $this->generateUrl('company_display_one_shipments_ca_imports', array(
            ...
            foreach ($fieldUsCustom->field_us_custom as $col) {
                ...
                if ($col == "hs_code_description") {
                    if (isset($r->getHit()["highlight"])) {
                        if ($col == "hs_code_description" and $r->getHit()["highlight"] and !empty($r->getHit()["highlight"])) {
