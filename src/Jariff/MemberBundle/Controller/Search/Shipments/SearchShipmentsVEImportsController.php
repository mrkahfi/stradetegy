...
use Jariff\MemberBundle\Form\SearchGlobal\Shipments\VEImports\FieldVEImportFormType;
 ...
 * @Route("/member")
 */
class SearchShipmentsVEImportsController extends BaseController
{
    ...
    /**
     * @Route(
     *   "/search-shipments-result/ve-imports/{page}",
     *   name     = "member_search_shipments_result_ve_imports",
     *   defaults = {
     *     "page"  : "1"
     *   },
     *   requirements = {
     ...
     * )
     *
     * @Template("JariffMemberBundle:Search\ShipmentsVeImports:results.html.twig")
     */
    ...
    $formFieldUsCustom = $this->createForm(new FieldVEImportFormType(), $fieldUsCustom, array(
        'action' => '',
        'method' => 'POST',
        ));
    ...
    /**
     * @Route(
     *   "/search-shipments-result-ajax/ve-imports/{page}",
     *   name     = "member_search_shipments_result_ve_imports_ajax",
     *   defaults = {
     *     "page"  : "1"
     *   },
     *   requirements = {
     ...
     * )
     *
     * @Template("JariffMemberBundle:Search\ShipmentsVeImports\Add:results_ajax.html.twig")
     */
    public function resultAjaxAction($page = 1)
    {
        ...
        $highlight = array(
            "highlight" => array(
                ...
                "fields" => array(
                    ...
                    "chapter_description" => array("number_of_fragments" => 0, "pre_tags" => array("<strong>"), "post_tags" => array("</strong>")),
                    )
                )
            );
        ...
        $searchable = new Index($client, 'stradetegy_ve_imports');
        ...
        $html = $view->render($res, function ($page) {
            return $this->generateUrl('member_search_shipments_result_ve_imports_ajax', array('page' => $page, 's_cache' => $this->s_cache_json));
        }, $options);
        ...
        $fieldUsCustom = new FieldUsCustom();
        if (empty($s_cache->column))
            $fieldUsCustom->field_us_custom = array("date","record","bl","chapter","chapter_description","hs_code","hs_code_description","importer","custom","transport_type",
                "embarq_port","origin_country","adq_country","gross_weight","net_weight",);
        else {
            $fieldUsCustom->field_us_custom = (is_array($s_cache->column)) ? $s_cache->column : array($s_cache->column);
        }
        ...
        return new Response(
            json_encode(
                array(
                    'success' => true,
                    'html_string' => $this->renderView('JariffMemberBundle:Search\ShipmentsVeImports\Add:results_ajax.html.twig', $params),
                    's_cache' => $this->s_cache_json,
                    'url_changes' => $this->generateUrl('member_search_shipments_result_ve_imports', array('page' => $page, 's_cache' => $this->s_cache_json))
    ...
    /**
     * @Route(
     *   "/search-shipments-columns-submit-ve-imports",
     *   name     = "member_search_columns_category_submit_ve_imports"
     * )
     * @Method("POST")
     *
     */
    public function submitCategoryAction(Request $request)
    {
        ...
        $form = $this->createForm(new FieldVEImportFormType(), $entity, array(
            'action' => '',
            'method' => 'POST',
            ));
        ...
        return $this->redirectUrl('member_search_shipments_result_ve_imports', array('s_cache' => $s_cache));
    ...
    /**
     * @Route(
     *   "/search-shipments-columns-show-ve-imports",
     *   name     = "member_search_columns_category_columns_show_ve_imports"
     * )
     *
     *
     */
    public function showChoiceColoumnAction(Request $request)
    {
        ...
        return new Response(
            json_encode(
                array(
                    'success' => true,
                    'html_string' => $this->renderView('JariffMemberBundle:Search\ShipmentsVeImports\Add\Form:field_column.html.twig', $params),
    ...
    /**
     * @Route(
     *   "/company-display-one-shipments-ve-imports/{index}/{type}/{objId}",
     *   name     = "company_display_one_shipments_ve_imports",
     *  options={"expose"=true}
     * )
     *
     *
    ...
    function displayOneShipmentsAction($index, $type, $objId)
    {
        ...
        return new Response(
            json_encode(
                array(
                    'success' => true,
                    'html_string' => $this->renderView('JariffMemberBundle:Search\ShipmentsVeImports\Add:modal_show_one_shipments.html.twig', array('entity' => $ret,"params" => $params)),
    ...
    /**
     * @Route(
     *   "/search-shipments-result-ajax/ve-imports/jsonp",
     *   name     = "member_search_shipments_result_ve_imports_ajax_jsonp",
     *   defaults = {
     *     "page"  : "1"
     *   },
     *   requirements = {
...
//        }
        ...
        if ($res->getNbResults() == 0) {
            ...
            $inbound->setVisitedPage($this->generateUrl('member_search_shipments_result_ve_imports', array('s_cache' => $this->s_cache_json)));
...
if ($this->get('session')->get('checkSubmit')) {
    ...
    if (!empty($cek)) {
        if (($cek->getTotal() + 1) > $active_subscription->getSearchValue()) {
            ...
            $urlChanges = $this->generateUrl('member_search_shipments_result_ve_imports', array('page' => $page, 's_cache' => $this->s_cache_json));
        ...
        foreach ($res->getCurrentPageResults()->getResults() as $r) {
            ...
            data-whatever="' . $r->getHit()["_source"]['importer'] . '"
            class="j-modal-company-one-shipments"
            data-toggle="modal" data-target="#myModal" data-backdrop="static"
            data-url="' . $this->generateUrl('company_display_one_shipments_ve_imports', array(
