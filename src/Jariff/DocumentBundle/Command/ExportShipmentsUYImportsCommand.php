...
class ExportShipmentsUYImportsCommand extends ContainerAwareCommand
{

    protected function configure()
    {
        $this
            ->setName('jariff:cron:export-shipments-uy-imports')
            ->setDescription('Mengeksekusi file export shipments UY IM')
            ->setHelp('<<<EOF
            <info>jariff:cron:export</info>
            Command digunakan untuk menyimpan data pencarian

            Syntax :
            php app/console jariff:cron:export-shipments-uy-imports
            EOF'
            );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $fp = fopen(getcwd() . '/lock/jariff-export-uy-imports.lock', 'w');
        ...
        if (flock($fp, LOCK_EX | LOCK_NB)) {
            ...
            echo "mulai\n";
            $exportDownloadQueque = $em->getRepository('JariffMemberBundle:ExportTools')->findOneBy(array('process' => 1,'collection' => 'uy-imports'));
            ...
            ->setTitle("Shipments Data UY Imports");
...
//            $highlight,
            ...
            $searchable = new Index($client, 'stradetegy-im-uy');
            ...
            $columns = array();
            $cell = $phpExcelObject->createSheet(0)->setTitle('Shipments UY Imports');
            $i = 0;
            foreach ($column as $col) {
                $columns[$util->uy_imports_column_view($col)] = $util->rowXls($i) . '1';
                $cell->setCellValue($util->rowXls($i) . '1', $util->uy_imports_column_view($col));
                $i++;
            }
