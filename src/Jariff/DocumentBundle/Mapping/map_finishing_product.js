...
var resSup1 = db.import_document_exporter_real.mapReduce(mapExporter, reduceExporter,
    { out: "import_document_product_finishing"});

db.import_document_exporter_real_part_1.find().forEach(
    function (obj) {
        db.import_document_product_finishing.update({'value.slug_company': obj._id},
            {
                $set: {
...
db.import_document_exporter_finishing.find().forEach(
    function (obj) {
        db.import_document_product_finishing.update({'value.slug_company': obj.value.slug_company},
            {
                $set: {
                    'value.company_join': obj.value.company_join
                }
