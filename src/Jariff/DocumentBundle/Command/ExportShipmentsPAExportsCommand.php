...
class ExportShipmentsPAExportsCommand extends ContainerAwareCommand
{

    protected function configure()
    {
        $this
            ->setName('jariff:cron:export-shipments-pa-exports')
            ->setDescription('Mengeksekusi file export shipments PA EX')
            ->setHelp('<<<EOF
            <info>jariff:cron:export</info>
            Command digunakan untuk menyimpan data pencarian

            Syntax :
            php app/console jariff:cron:export-shipments-pa-exports
            EOF'
            );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $fp = fopen(getcwd() . '/lock/jariff-export-pa-exports.lock', 'w');
        ...
        if (flock($fp, LOCK_EX | LOCK_NB)) {
            ...
            echo "mulai\n";
            $exportDownloadQueque = $em->getRepository('JariffMemberBundle:ExportTools')->findOneBy(array('process' => 1,'collection' => 'pa-exports'));
            ...
            ->setTitle("Shipments Data PA Exports");
...
//            $highlight,
            ...
            $searchable = new Index($client, 'stradetegy-ex-pa');
            ...
            $columns = array();
            $cell = $phpExcelObject->createSheet(0)->setTitle('Shipments PA Exports');
            $i = 0;
            foreach ($column as $col) {
                $columns[$util->pa_exports_column_view($col)] = $util->rowXls($i) . '1';
                $cell->setCellValue($util->rowXls($i) . '1', $util->pa_exports_column_view($col));
                $i++;
            }
