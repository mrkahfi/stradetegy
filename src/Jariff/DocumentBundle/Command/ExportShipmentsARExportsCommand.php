...
class ExportShipmentsARExportsCommand extends ContainerAwareCommand
{

    protected function configure()
    {
        $this
        ->setName('jariff:cron:export-shipments-ar-exports')
        ->setDescription('Mengeksekusi file export shipments AR EX')
        ->setHelp('<<<EOF
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $fp = fopen(getcwd() . '/lock/jariff-export-ar-exports.lock', 'w');
        ...
        if (flock($fp, LOCK_EX | LOCK_NB)) {
            ...
            echo "mulai\n";
            $exportDownloadQueque = $em->getRepository('JariffMemberBundle:ExportTools')->findOneBy(array('process' => 1,'collection' => 'ar-exports'));
            ...
            ->setTitle("Shipments Data AR Exports");
...
//            $highlight,
            ...
            foreach ($column as $col) {
                $columns[$util->ar_exports_column_view($col)] = $util->rowXls($i) . '1';
                $cell->setCellValue($util->rowXls($i) . '1', $util->ar_exports_column_view($col));
                $i++;
            }
