...
use Jariff\MemberBundle\Form\SearchGlobal\Shipments\JNPTImports\FieldJNPTImportFormType;
 ...
 * @Route("/member")
 */
class SearchShipmentsJNPTImportsController extends BaseController
{
    ...
    /**
     * @Route(
     *   "/search-shipments-result/jpnt-imports/{page}",
     *   name     = "member_search_shipments_result_jnpt_imports",
     *   defaults = {
     *     "page"  : "1"
     *   },
     *   requirements = {
     ...
     * )
     *
     * @Template("JariffMemberBundle:Search\ShipmentsJnptImports:results.html.twig")
     */
    public function resultAction($page = 1)
    {
        ...
        $formFieldUsCustom = $this->createForm(new FieldJNPTImportFormType(), $fieldUsCustom, array(
            'action' => '',
            'method' => 'POST',
        ));
    ...
    /**
     * @Route(
     *   "/search-shipments-result-ajax/jnpt-imports/{page}",
     *   name     = "member_search_shipments_result_jnpt_imports_ajax",
     *   defaults = {
     *     "page"  : "1"
     *   },
     *   requirements = {
     ...
     * )
     *
     * @Template("JariffMemberBundle:Search\ShipmentsJnptImports\Add:results_ajax.html.twig")
     */
    public function resultAjaxAction($page = 1)
    {
        ...
        $highlight = array(
            "highlight" => array(
                ...
                "fields" => array(
                    ...
                    "item_description" => array("number_of_fragments" => 0, "pre_tags" => array("<strong>"), "post_tags" => array("</strong>")),
                )
            )
        );
        ...
        $searchable = new Index($client, 'stradetegy_jnpt_imports');
        ...
        $html = $view->render($res, function ($page) {
            return $this->generateUrl('member_search_shipments_result_jnpt_imports_ajax', array('page' => $page, 's_cache' => $this->s_cache_json));
        }, $options);
        ...
        $fieldUsCustom = new FieldUsCustom();
        if (empty($s_cache->column))
            $fieldUsCustom->field_us_custom = array("date", "importer_name",
                "importer_address", "item_description","supplier_name","origin_port");
        else {
            $fieldUsCustom->field_us_custom = (is_array($s_cache->column)) ? $s_cache->column : array($s_cache->column);
        }
        ...
        return new Response(
            json_encode(
                array(
                    'success' => true,
                    'html_string' => $this->renderView('JariffMemberBundle:Search\ShipmentsJnptImports\Add:results_ajax.html.twig', $params),
                    's_cache' => $this->s_cache_json,
                    'url_changes' => $this->generateUrl('member_search_shipments_result_jnpt_imports', array('page' => $page, 's_cache' => $this->s_cache_json))
    ...
    /**
     * @Route(
     *   "/search-shipments-columns-submit-jnpt-imports",
     *   name     = "member_search_columns_category_submit_jnpt_imports"
     * )
     * @Method("POST")
     *
     */
    public function submitCategoryAction(Request $request)
    {
        ...
        $form = $this->createForm(new FieldJNPTImportFormType(), $entity, array(
            'action' => '',
            'method' => 'POST',
        ));
        ...
        return $this->redirectUrl('member_search_shipments_result_jnpt_imports', array('s_cache' => $s_cache));

    }

    /**
     * @Route(
     *   "/search-shipments-columns-show-jnpt-imports",
     *   name     = "member_search_columns_category_columns_show_jnpt_imports"
     * )
     *
     *
     */
    public function showChoiceColoumnAction(Request $request)
    {
        ...
        return new Response(
            json_encode(
                array(
                    'success' => true,
                    'html_string' => $this->renderView('JariffMemberBundle:Search\ShipmentsCrImports\Add\Form:field_column.html.twig', $params),
    ...
    /**
     * @Route(
     *   "/company-display-one-shipments-jnpt-imports/{index}/{type}/{objId}",
     *   name     = "company_display_one_shipments_jnpt_imports",
     *  options={"expose"=true}
     * )
     *
     *
    ...
    function displayOneShipmentsAction($index, $type, $objId)
    {
        ...
        return new Response(
            json_encode(
                array(
                    'success' => true,
                    'html_string' => $this->renderView('JariffMemberBundle:Search\ShipmentsJnptImports\Add:modal_show_one_shipments.html.twig', array('entity' => $ret,'params' => $params)),
    ...
    /**
     * @Route(
     *   "/search-shipments-result-ajax/jnpt-imports/jsonp",
     *   name     = "member_search_shipments_result_jnpt_imports_ajax_jsonp",
     *   defaults = {
     *     "page"  : "1"
     *   },
     *   requirements = {
    ...
    public function resultAjaxJsonpAction()
    {
        ...
        $queryDSL = $this->queryDSL($s_cache, "date", "item_description", $fields, $size, $page);
...
//        }
        ...
        if ($res->getNbResults() == 0) {
            ...
            $inbound->setVisitedPage($this->generateUrl('member_search_shipments_result_jnpt_imports', array('s_cache' => $this->s_cache_json)));
        ...
        if ($maxTime > 1) {
            ...
            $urlChanges = $this->generateUrl('member_search_shipments_result_jnpt_imports', array('page' => $page, 's_cache' => $this->s_cache_json));
        ...
        foreach ($res->getCurrentPageResults()->getResults() as $r) {
            ...
            $columnMd['detail_company'] = ' <a href="#"
                                                       data-whatever="' . $r->getHit()["_source"]['importer_name'] . '"
                                                       class="j-modal-company-one-shipments"
                                                       data-toggle="modal" data-target="#myModal" data-backdrop="static"
                                                       data-url="' . $this->generateUrl('company_display_one_shipments_jnpt_imports', array(
