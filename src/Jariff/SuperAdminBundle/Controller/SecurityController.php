...
class SecurityController extends BaseController
{
    ...
    /**
     * @Route("/super-admin-reset", name="super_admin_reset")
     * @Template()
     */
    ...
    /**
     * @Route("/reset/password", name="super_admin_reset_password")
     * @Template()
     */
    public function resetpasswordAction()
    {
        ...
        if($request->isMethod('POST')){
            ...
            if ($form->isValid()) {
                ...
                $client = $this->repo('JariffSuperAdminBundle:SuperAdmin')->findOneByUsername($data['username']);
                $message = \Swift_Message::newInstance()
                    ...
                    ->setBody(
                        $this->renderView('JariffSuperAdminBundle:Email:reset.html.twig', array('data' => $client), 'text/html'))
                    ->addPart(
                        $this->renderView('JariffSuperAdminBundle:Email:reset.txt.twig', array('data' => $client)))
                ;
    ...
    /**
     * @Route("/reset/password2", name="super_admin_reset_password2")
     * @Template()
     */
    ...
    /**
     * @Route("/reset/password/email/{username}/{email}/{password}", name="super_admin_reset_password_inemail")
     * @Template()
     */
    public function resetpasswordinemailAction($username,$email,$password)
    {
        $request = $this->getRequest();
        $client = $this->repo('JariffSuperAdminBundle:Admin')->findOneByUsername($username);
        if(
            !$client or
        ...
        if($request->isMethod('POST')){
            ...
            if ($form->isValid()) {
                ...
                $message = \Swift_Message::newInstance()
                    ...
                    ->setTo(array($client->getProfile()->getEmail()))
                    ->setBody(
                        $this->renderView('JariffSuperAdminBundle:Email:newpassword.html.twig', array('data' => $data), 'text/html'))
                    ->addPart(
                        $this->renderView('JariffSuperAdminBundle:Email:newpassword.txt.twig', array('data' => $data)))                        
                ;
                $this->get('mailer')->send($message);
