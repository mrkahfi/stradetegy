...
class ExportShipmentsCAImportsCommand extends ContainerAwareCommand
{

    protected function configure()
    {
        $this
            ->setName('jariff:cron:export-shipments-ca-imports')
            ->setDescription('Mengeksekusi file export shipments CA IM')
            ->setHelp('<<<EOF
            <info>jariff:cron:export</info>
            Command digunakan untuk menyimpan data pencarian

            Syntax :
            php app/console jariff:cron:export-shipments-ca-imports
            EOF'
            );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $fp = fopen(getcwd() . '/lock/jariff-export-ca-imports.lock', 'w');
        ...
        if (flock($fp, LOCK_EX | LOCK_NB)) {
            ...
            echo "mulai\n";
            $exportDownloadQueque = $em->getRepository('JariffMemberBundle:ExportTools')->findOneBy(array('process' => 1,'collection' => 'ca-imports'));
            ...
            ->setTitle("Shipments Data CA Imports");
...
//            $highlight,
            ...
            $searchable = new Index($client, 'stradetegy-im-ca');
            ...
            $columns = array();
            $cell = $phpExcelObject->createSheet(0)->setTitle('Shipments CA Imports');
            $i = 0;
            foreach ($column as $col) {
                $columns[$util->ca_imports_column_view($col)] = $util->rowXls($i) . '1';
                $cell->setCellValue($util->rowXls($i) . '1', $util->ca_imports_column_view($col));
                $i++;
            }
