...
use Jariff\MemberBundle\Form\SearchGlobal\Shipments\MXImports\FieldMXImportFormType;
 ...
 * @Route("/member")
 */
class SearchShipmentsMXImportsController extends BaseController
{
    ...
    /**
     * @Route(
     *   "/search-shipments-result/mx-imports/{page}",
     *   name     = "member_search_shipments_result_mx_imports",
     *   defaults = {
     *     "page"  : "1"
     *   },
     *   requirements = {
     ...
     * )
     *
     * @Template("JariffMemberBundle:Search\ShipmentsMxImports:results.html.twig")
     */
    public function resultAction($page = 1)
    {
        ...
        if (empty($s_cache->column))
            $fieldUsCustom->field_us_custom = array("date", "hs_code",
                "hs_product_dectiption", "product_schedule_b_code","product_decription_by_schedule_b_code_mexico","way_of_transport");
        else {
            $fieldUsCustom->field_us_custom = (is_array($s_cache->column)) ? $s_cache->column : array($s_cache->column);
        }

        $formFieldUsCustom = $this->createForm(new FieldMXImportFormType(), $fieldUsCustom, array(
            'action' => '',
            'method' => 'POST',
            ));
    ...
    /**
     * @Route(
     *   "/search-shipments-result-ajax/mx-imports/{page}",
     *   name     = "member_search_shipments_result_mx_imports_ajax",
     *   defaults = {
     *     "page"  : "1"
     *   },
     *   requirements = {
     ...
     * )
     *
     * @Template("JariffMemberBundle:Search\ShipmentsMxImports\Add:results_ajax.html.twig")
     */
    public function resultAjaxAction($page = 1)
    {
        ...
        $highlight = array(
            "highlight" => array(
                ...
                "fields" => array(
                    ...
                    "product_decription_by_schedule_b_code_mexico" => array("number_of_fragments" => 0, "pre_tags" => array("<strong>"), "post_tags" => array("</strong>")),
                    )
                )
            );
        ...
        $searchable = new Index($client, 'stradetegy_mx_imports');
        ...
        $html = $view->render($res, function ($page) {
            return $this->generateUrl('member_search_shipments_result_mx_imports_ajax', array('page' => $page, 's_cache' => $this->s_cache_json));
        }, $options);
        ...
        return new Response(
            json_encode(
                array(
                    'success' => true,
                    'html_string' => $this->renderView('JariffMemberBundle:Search\ShipmentsMxImports\Add:results_ajax.html.twig', $params),
                    's_cache' => $this->s_cache_json,
                    'url_changes' => $this->generateUrl('member_search_shipments_result__imports', array('page' => $page, 's_cache' => $this->s_cache_json))
    ...
    /**
     * @Route(
     *   "/search-shipments-columns-submit-mx-imports",
     *   name     = "member_search_columns_category_submit_mx_imports"
     * )
     * @Method("POST")
     *
     */
    public function submitCategoryAction(Request $request)
    {
        ...
        $form = $this->createForm(new FieldMXImportFormType(), $entity, array(
            'action' => '',
            'method' => 'POST',
            ));
        ...
        return $this->redirectUrl('member_search_shipments_result_mx_imports', array('s_cache' => $s_cache));

    }

        /**
     * @Route(
     *   "/search-shipments-columns-show-mx-imports",
     *   name     = "member_search_columns_category_columns_show_mx_imports"
     * )
     *
     *
     */
        public function showChoiceColoumnAction(Request $request)
        {
            ...
            $fieldUsCustom = new FieldUsCustom();
            if (empty($s_cache->column))
                $fieldUsCustom->field_us_custom = array("date", "importer",
                    "destination_country", "comercial_description","unit_of_meassure","packages");
            else {
                $fieldUsCustom->field_us_custom = (is_array($s_cache->column)) ? $s_cache->column : array($s_cache->column);
            }
            ...
            return new Response(
                json_encode(
                    array(
                        'success' => true,
                        'html_string' => $this->renderView('JariffMemberBundle:Search\ShipmentsMxImports\Add\Form:field_column.html.twig', $params),
    ...
    /**
     * @Route(
     *   "/company-display-one-shipments-mx-imports/{index}/{type}/{objId}",
     *   name     = "company_display_one_shipments_mx_imports",
     *  options={"expose"=true}
     * )
     *
     *
    ...
    function displayOneShipmentsAction($index, $type, $objId)
    {
        ...
        return new Response(
            json_encode(
                array(
                    'success' => true,
                    'html_string' => $this->renderView('JariffMemberBundle:Search\ShipmentsMxImports\Add:modal_show_one_shipments.html.twig', array('entity' => $ret,"params" => $params)),
    ...
    /**
     * @Route(
     *   "/search-shipments-result-ajax/mx-imports/jsonp",
     *   name     = "member_search_shipments_result_mx_imports_ajax_jsonp",
     *   defaults = {
     *     "page"  : "1"
     *   },
     *   requirements = {
    ...
    $fields = array(
        "hs_product_dectiption",
        "product_schedule_b_code",
        "product_decription_by_schedule_b_code_mexico",
        "way_of_transport",
        "country_of_origin");


    $queryDSL = $this->queryDSL($s_cache, "date", "", $fields, $size, $page);
    ...
    $fieldUsCustom = new FieldUsCustom();
    if (empty($s_cache->column))
        $fieldUsCustom->field_us_custom = array( "date_yyyy_mm",
            "date",
            "hs_code",
            "hs_product_dectiption",
            "product_schedule_b_code",
...
//        }
    ...
    if ($res->getNbResults() == 0) {
        ...
        $inbound->setVisitedPage($this->generateUrl('member_search_shipments_result_mx_imports', array('s_cache' => $this->s_cache_json)));
...
if ($maxTime > 1) {
    ...
    $urlChanges = $this->generateUrl('member_search_shipments_result_mx_imports', array('page' => $page, 's_cache' => $this->s_cache_json));
...
foreach ($res->getCurrentPageResults()->getResults() as $r) {
    ...
    data-url="' . $this->generateUrl('company_display_one_shipments_mx_imports', array(
